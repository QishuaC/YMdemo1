...
Page({
  ...
  updateVisibleVideos(currentIndex) {
    const state = this.stateManager.getState();
    const allVideos = state.videos;
    const range = 1;
    
    const visibleVideos = allVideos.map((video, index) => {
      const isVisible = Math.abs(index - currentIndex) <= range;
      return {
        ...video,
        _shouldRender: isVisible
      };
    });
    
    this.stateManager.setVisibleVideos(visibleVideos);
    this.setData({
  ...
  playVideo(idx) {
    ...
    this.playerManager.play(safeIdx).then(() => {
      ...
      this.stateManager.updateVideoPlaying(safeIdx);
      
      const updates = {};
      if (prevIdx !== undefined && prevIdx !== null && prevIdx >= 0) {
        updates[`videos[${prevIdx}].playing`] = false;
      }
      updates[`videos[${safeIdx}].playing`] = true;
  ...
  pauseVideo(idx) {
    ...
    this.stateManager.pauseAllVideos();
    this.setData({
      [`videos[${idx}].playing`]: false
    });
  },

  pauseAllVideos() {
    const state = this.stateManager.getState();
    this.playerManager.pauseAll(state.currentIdx);
  },
  ...
  async doLike(e) {
    ...
    const allVideoIndex = state.allVideos.findIndex(v => v.id === videoId);
    
    const updates = {};
    if (videoIndex >= 0) {
      updates[`videos[${videoIndex}].liked`] = newLiked;
      updates[`videos[${videoIndex}].likes`] = newLikes;
    }
    if (allVideoIndex >= 0) {
      updates[`allVideos[${allVideoIndex}].liked`] = newLiked;
      updates[`allVideos[${allVideoIndex}].likes`] = newLikes;
    }
  ...
  rollbackLike(videoId, liked, likes) {
    ...
    const updates = {};
    if (videoIndex >= 0) {
      updates[`videos[${videoIndex}].liked`] = liked;
      updates[`videos[${videoIndex}].likes`] = likes;
    }
    if (allVideoIndex >= 0) {
      updates[`allVideos[${allVideoIndex}].liked`] = liked;
      updates[`allVideos[${allVideoIndex}].likes`] = likes;
    }
  ...
  updateCommentCount(count) {
    ...
    const videoIndex = state.videos.findIndex(v => String(v.id) === String(videoId));
    const allVideoIndex = state.allVideos.findIndex(v => String(v.id) === String(videoId));
    
    const updates = {
      currentCommentCount: state.currentCommentCount
    };
    if (videoIndex >= 0) {
      updates[`videos[${videoIndex}].comments`] = count;
    }
    if (allVideoIndex >= 0) {
      updates[`allVideos[${allVideoIndex}].comments`] = count;
    }
  ...
  onShareAppMessage() {
    ...
    return buildShareConfig({
      ...
      path: '/packageContent/pages/video/video?id=' + (video ? video.id : ''),
